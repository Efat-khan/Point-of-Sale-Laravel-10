...
<div class="page-content">
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    ...
                    <h4 class="mb-sm-0">Edit Invoice ({{$invoice->invoice_no}})</h4>
                    @elseif($invoice->invoice_type ==='challan')
                    <h4 class="mb-sm-0">Edit Challan ({{$invoice->invoice_no}})</h4>
                    @elseif($invoice->invoice_type ==='quotation')
                    <h4 class="mb-sm-0">Edit Quotation ({{$invoice->invoice_no}})</h4>
                    @elseif($invoice->invoice_type ==='draft')
                    <h4 class="mb-sm-0">Edit Draft ({{$invoice->invoice_no}})</h4>
                    @endif
                    ...
                    <form method="post" action="{{ route('invoice.update') }}" onsubmit="return confirmAction(event)">
                        ...
                        <div class="card-body pb-0">
                            <div class="row">
                                ...
                                <!-- <div class="col-md-2 text-left">
                                    ...
                                    <input class="form-control" type="text" name="dn_no" value="{{ $invoice->dn_no }}" id="dn_no" readonly style="background-color:#ddd; ">
                                </div> -->
                                <div class="col-md-3 text-left">
                                    <label for="example-text-input" class="col-form-label">WO No.</label>
                                    <input class="form-control" type="text" name="wo_no" value="{{ $invoice->wo_no }}" id="wo_no" placeholder="Enter WO No">
                                </div>
                                <div class="form-group col-md-5 pb-0">
                                    ...
                                    <select name="customer_id" id="customer_id" class="form-select">
                                        ...
                                        <option value="{{ $customer->id }}" {{ $payment->customer_id == $customer->id ?'selected': '' }}>{{ $customer->name }} - {{$customer->mobile_no }}</option>
                                        @endforeach
                                    </select><br>
                                </div>
                                <div class="col-md-2">
                        ...
                        <div class="card-body">
                            <table class="table-sm table-bordered" width="100%" style="border-color: #ddd;">
                                ...
                                <tbody id="addRow" class="addRow">
                                    ...
                                    <tr class="delete_add_more_item" id="delete_add_more_item">
                                        ...
                                        <input type="hidden" name="product_id[]" value="{{$invoice_details->product_id ?? ''}}">
                                        <td>{{ $invoice_details['product']->name ?? '' }}-{{$invoice_details['product']['brand']->name ?? ''}}({{$invoice_details['product']->tax_type}})</td>
                                        <td>
                                            ...
                                            <input type="text" class="form-control stock_quantity text-right" name="stock_quantity[]" value="{{$invoice_details['product']->quantity}}" readonly>
                                        </td>
                                        <td>
                                            <input type="text" class="form-control product_buying_price text-right" name="product_buying_price[]" value="{{$invoice_details['product']->product_buying_price}}" hidden>
                                            <input type="text" class="form-control unit_price text-right" name="unit_price[]" value="{{$invoice_details->unit_price}}">
                                            <input type="text" class="form-control unit_price_code text-right" name="product_price_code" value="{{$invoice_details['product']->product_price_code}}" hidden>
                                        </td>
                                        <!-- TAX  -->
                                        @php
                                        $tax_ids = json_decode($invoice_details->tax_type);
                                        ...
                                        <td class="discount-row">
                                            <select name="product_tax[{{$invoice_details->product_id}}][]" multiple="multiple" class="form-group form-select product_tax">
                                        ...
                                        <td class="discount-row">
                                            <div class="form-check form-switch">
                                                <input class="form-check-input discount_per_product" type="checkbox" {{ $invoice_details->discount_type == 'percentage' ? 'checked' : '' }}>
                                                <label class="form-check-label discount_label" for="flexSwitchCheckDefault">{{ $invoice_details->discount_type == 'percentage' ? '% Percent' : 'Fixed' }}</label>
                                            </div>
                                        ...
                                        <td>
                                            ...
                                            @php
                                            $buying_price = $invoice_details['product']->product_buying_price * $invoice_details->selling_qty;
                                            @endphp
                                            ...
                                            <input type="text" class="form-control product_tax_amount text-right" id="" name="product_tax_amount[]" value="{{$invoice_details->tax_amount ?? 0 }}" readonly>
                                            @php
                                            $product_price_for_tax = $invoice_details->unit_price * $invoice_details->selling_qty;
                                            @endphp
                                            ...
                                            <!-- <input type="text" class="form-control product_price_for_tax text-right" id="" name="product_price_for_tax[]" value="{{$invoice_details->unite_price * $invoice_details->selling_qty}}" readonly hidden> -->
                                        </td>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <!-- Invoice Tax -->
                                        @php
                                        $tax_ids = json_decode($invoice->invoice_tax_type);
                                        $selected_tax = App\Models\Tax::whereIn('id', $tax_ids)->get();
                                        $unselected_tax = App\Models\Tax::whereNotIn('id', $tax_ids)->get();
                                        @endphp
                                        ...
                                        <td>
                                            ...
                                            <input type="text" name="invoice_tax_amount" id="invoice_tax_amount" class="form-control " value="{{ $invoice->invoice_tax_amount ?? 0}}" hidden>
                                        </td>
                                    </tr>
                                    <tr>
                                        <!-- Additional Fee -->
                                        @php
                                        $additional_fee_ids = json_decode($invoice->additional_charge_type);
                                        ...
                                        <td>
                                            ...
                                            <input type="text" name="total_additional_fees_amount" id="total_additional_fees_amount" class="form-control" value="{{$invoice->additional_charge_amount ?? 0}}" readonly>
                                        </td>
                                    </tr>
                                    <tr>
                                        {{-- 
                                    ...
                                    <tr>
                                        {{-- 
                                            <td colspan="1" class="text-start">Total Profit Code</td>
                                            <td colspan="2">
                            ...
                            <div class="form-group">
                                ...
                                <button type="submit" name="saveBtn" value="1" class="btn btn-primary" id="quotationButton"><i class="fas fa-check-circle"></i> Quotation</button>
                                <button type="submit" name="saveBtn" value="4" class="btn btn-primary" id="challanButton"><i class="fas fa-check-circle"></i> Challan</button>
    ...
    <script id="document-template" type="text/x-handlebars-template">
        <tr class="delete_add_more_item" id="delete_add_more_item">
            ...
            <td>
                <input type="number"  min="1"  class="form-control selling_qty text-right" name="selling_qty[]" id="selling_qty" value="0"> 
        ...
        <td>
            ...
            <input type="text" class="form-control product_tax_amount text-right" id="product_tax_amount" name="product_tax_amount[]" value="0" readonly>
            <input type="text" class="form-control product_price_for_tax text-right" id="" name="product_price_for_tax[]" value="0" hidden>
        </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const challanButton = document.getElementById('challanButton');
                    ...
                    success: function(data) {
                        ...
                        if (quantity > 0) {
                            ...
                            var data = {
                                ...
                                product_buying_price: product_list.product_buying_price,
